...
export const ConfigurationForm: React.FC<ConfigurationFormProps> = ({ onSubmit, isLoading }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl relative overflow-visible z-20 transition-colors duration-300">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-2 relative" ref={dropdownRef}>
          <label className="text-sm font-medium text-slate-500 dark:text-slate-400 ml-1">Target Country</label>
          <div className="relative">
            <div 
              className={`w-full bg-slate-50 dark:bg-slate-900 border ${isCountryOpen ? 'border-indigo-500 ring-2 ring-indigo-500/20' : 'border-slate-300 dark:border-slate-700'} text-slate-900 dark:text-slate-200 rounded-xl px-4 py-3 flex items-center justify-between cursor-text transition-all`}
              onClick={() => {
            ...
            >
              ...
              <div className="text-slate-500 flex-shrink-0 ml-2">
                {isCountryOpen ? <Search className="w-4 h-4 text-indigo-500 dark:text-indigo-400" /> : <ChevronRight className="w-4 h-4 rotate-90" />}
              </div>
            </div>

            {/* Dropdown List */}
            {isCountryOpen && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto custom-scrollbar z-50">
                  ...
                  filteredCountries.map((c) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex flex-col justify-center group ${country === c ? 'bg-indigo-50 dark:bg-indigo-500/10' : ''}`}
                      >
                        <div className="flex items-center justify-between w-full">
                          <span className={country === c ? 'text-indigo-600 dark:text-indigo-300 font-medium' : 'text-slate-700 dark:text-slate-300'}>
                            {c}
                          </span>
                          {country === c && <Check className="w-3 h-3 text-indigo-500 dark:text-indigo-400" />}
                        </div>
                        {matchedAlias && (
                           <span className="text-xs text-indigo-500/70 dark:text-indigo-400/70 italic mt-0.5">Matches: "{matchedAlias}"</span>
                        )}
                      </button>
                    );
                  })
          ...
          {/* Variant Selector */}
          <div className="mt-4 animate-fade-in px-1 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50">
             <div className="flex items-center justify-between mb-3">
               ...
               <button
                  ...
                  className={`text-[10px] uppercase font-bold px-2 py-1 rounded flex items-center gap-1 transition-colors ${isRemixMode ? 'bg-pink-500/20 text-pink-600 dark:text-pink-300' : 'bg-slate-200 dark:bg-slate-800 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
               >
             ...
             {isRemixMode && (
               <div className="mb-2 text-[10px] text-pink-500/80 dark:text-pink-300/80 italic">
                    ...
                    <div key={v} className="relative group">
                      <button
                        ...
                        className={`
                          ...
                          ${isSelected
                            ? (isRemixMode ? 'bg-pink-500/20 border-pink-500 text-pink-700 dark:text-pink-300 shadow-sm' : 'bg-indigo-500/20 border-indigo-500 text-indigo-700 dark:text-indigo-300 shadow-sm')
                            : 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-slate-400 dark:hover:border-slate-500 hover:text-slate-800 dark:hover:text-slate-300'}
                        `}
                      >
                        {v}
                      </button>
                      {isCustom && (
                        <button
                          ...
                          className="absolute -top-1.5 -right-1.5 bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-300 rounded-full p-0.5 hover:bg-red-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100"
                        >
                          <X className="w-2 h-2" />
                        </button>
                      )}
                ...
                {isRemixMode && (
                  <div className="flex items-center">
                    <input
                      ...
                      className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-l-lg px-2 py-1.5 text-xs text-slate-900 dark:text-slate-200 w-24 focus:outline-none focus:border-pink-500"
                    />
                    <button
                      type="button"
                      onClick={handleAddCustomVariant}
                      className="bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-700 border-l-0 rounded-r-lg px-2 py-1.5 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300"
                    >
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-slate-500 dark:text-slate-400 ml-1">Document Type</label>
              ...
              <button
                ...
                className={`px-4 py-3 rounded-xl text-sm font-medium text-left transition-all border ${
                  documentType === type
                    ...
                    : 'bg-slate-50 dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-slate-400 dark:hover:border-slate-600'
                }`}
              >
                {type}
              </button>
        ...
        {/* Format Selection */}
        <div className="space-y-2">
          <label className="text-sm font-medium text-slate-500 dark:text-slate-400 ml-1">File Format</label>
              ...
              >
                ...
                <div className={`
                  ...
                  ${format === f 
                    ...
                    : 'bg-slate-50 dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-slate-400 dark:hover:border-slate-600'}
                `}>
        ...
        <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-slate-700/50">
           <label className="text-sm font-medium text-slate-500 dark:text-slate-400 ml-1 flex items-center gap-2">
           ...
           <div className="relative">
            <select
              ...
              disabled={isLoading}
              className="w-full appearance-none bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 hover:border-purple-500/50 text-slate-900 dark:text-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all disabled:opacity-50"
            >
